                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 text-center">
                                <div class="avatar-wrapper mb-3">
                                    <img :src="staff.avatar || defaultAvatar" alt="Avatar" class="rounded-circle img-fluid mb-3"
                            ...
                            <div class="col-md-9">
                                ...
                                <div class="profile-body">
                                    <div class="info-section">
                                        ...
                                        <div class="info-grid">
                                            <div class="info-item">
                                                <span class="info-icon"><Icon name="ic:outline-email" size="18" /></span>
                                                <div>
                                                    <p class="info-label">Email</p>
                                                    <p class="info-value">{{ staff.email ?? '---' }}</p>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="info-value">{{ staff.phone_number ?? '---' }}</p>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="info-value">{{ formatDate(staff.date_of_birth) ?? '---' }}</p>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="info-value">{{ staff.gender ?? '---' }}</p>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                                <span class="info-icon"><Icon name="tdesign:location" size="18" /></span>
                                                <div>
                                                    <p class="info-label">Địa chỉ</p>
                                                    <p class="info-value">{{ staff.address ?? '---' }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="info-section">
                                        ...
                                        <div class="note-box">
                                            <p style="margin: unset;">{{ staff.notes ?? 'Không có ghi chú' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-12">
                <div class="card shadow">
                    <div class="card-header bg-white">
                        <h5 class="mb-0">Danh sách tòa nhà quản lý</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-hover">
                                <thead class="table-light sticky-top">
                                    <tr>
                                        ...
                                        <th style="width: 15%;">Loại tòa nhà</th>
                                        <th style="width: 10%;">Vai trò</th>
                                        <th style="width: 10%;">Trạng thái</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="building in staff.staff_assignment" :key="building.staff_assignment_id">
                                        <td>{{ building.buildings[0].name }}</td>
                                        <td>{{ building.buildings[0].address }}</td>
                                        <td>{{ building.buildings[0].floors }}</td>
                                        <td>{{ building.buildings[0].total_area }}(m²)</td>
                                        <td>{{ getBuildingType(building.buildings[0].building_type) }}</td>
                                        <td>{{ getRole(building.role) }}</td>
                                        <td>
                                            <span
                                            :class="{
                                            'badge bg-primary': building.buildings[0].status === 0,
                                            'badge bg-secondary': building.buildings[0].status === 1
                                            }"
                                            >
                                                {{
                                                    building.buildings[0].status === 0
                                                    ? 'Đang hoạt động'
                                                    : building.buildings[0].status === 1
                                                    ? 'Ngừng hoạt động'
                                                    : '----'
                                                }}
                                            </span>
...
const getRole = (role) => {
    ...
    if (role === 'monitor') return 'Giám sát'
}
...
const getStaffDetail = async () => {
    const data = await staffStore.detailStaff(staff_id)
    if (data) {
        staff.value = data
